...
def test_v0294_activation_policy_is_bounded():
    ...
    assert (
        policy["documentation_policy"]["current_release_name"]
...
def test_v0294_release_documents_are_current():
    notes = (ROOT / "RELEASE_NOTES.md").read_text(encoding="utf-8")
    assert "v0.29.4 — Windows Restricted Execution" in notes
    assert "restricted_token_attested = true" in notes
    assert "low_integrity_attested = false" in notes

    for rel in ("README.md", "README.vi.md", "README.en.md"):
        text = (ROOT / rel).read_text(encoding="utf-8")
        assert "v0.29.4" in text
        assert "Windows Restricted Execution" in text

    node = (
        ROOT
        / ".agents/docs/WINDOWS_RESTRICTED_EXECUTION_V0294.md"
    ).read_text(encoding="utf-8")
    assert "restricted_token_attested = true" in node
    assert "low_integrity_attested = false" in node
