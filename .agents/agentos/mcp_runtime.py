...
from .mcp_v0271 import TOOLS as V0271_TOOLS, TOOL_NAMES as V0271_TOOL_NAMES, dispatch as dispatch_v0271_tool
from .mcp_v0272 import TOOLS as V0272_TOOLS, TOOL_NAMES as V0272_TOOL_NAMES, dispatch as dispatch_v0272_tool
from .mcp_v0273 import TOOLS as V0273_TOOLS, TOOL_NAMES as V0273_TOOL_NAMES, dispatch as dispatch_v0273_tool
...
def _merge_tools() -> tuple[list[dict[str, Any]], set[str], set[str]]:
    ...
    for definition in [*CORE_TOOLS, *FEATURE_TOOLS, *V0252_TOOLS, *V0253_TOOLS, *V0254_TOOLS, *V0255_TOOLS, *V0260_TOOLS, *V0261_TOOLS, *V0262_TOOLS, *V0263_TOOLS, *V0270_TOOLS, *V0271_TOOLS, *V0272_TOOLS, *V0273_TOOLS, *V0280_TOOLS, *V0290_TOOLS, HEALTH_TOOL]:
...
def _health(root: Path, task_id: str | None, session_id: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "extension_readonly_tool_count": len(FEATURE_TOOLS) + len(V0252_TOOLS) + len(V0253_TOOLS) + len(V0254_TOOLS) + len(V0255_TOOLS) + len(V0260_TOOLS) + len(V0261_TOOLS) + len(V0262_TOOLS) + len(V0263_TOOLS) + len(V0270_TOOLS) + len(V0271_TOOLS) + len(V0272_TOOLS) + len(V0273_TOOLS) + len(V0280_TOOLS) + len(V0290_TOOLS) - 1,
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if name == HEALTH_TOOL["name"]:
                ...
                continue
            if name in V0290_TOOL_NAMES:
                value = dispatch_v0290_tool(root, name, arguments)
                _reply(identifier, _tool_result(value))
                continue
            if name in V0280_TOOL_NAMES:
                value = dispatch_v0280_tool(root, name, arguments)
                _reply(identifier, _tool_result(value))
                continue
            if name in V0273_TOOL_NAMES:
                value = dispatch_v0273_tool(root, name, arguments)
                _reply(identifier, _tool_result(value))
                continue
            if name in V0272_TOOL_NAMES:
                value = dispatch_v0272_tool(root, name, arguments)
                _reply(identifier, _tool_result(value))
                continue
            if name in V0271_TOOL_NAMES:
                value = dispatch_v0271_tool(root, name, arguments)
                _reply(identifier, _tool_result(value))
                continue
