#!/usr/bin/env python3
"""Validate the AgentOS v0.23.1 full release tree and package metadata."""
...
def verify_manifest(root: Path) -> dict:
    """Verify MANIFEST.json and CHECKSUMS.sha256 without helper scripts."""
...
def main() -> None:
    ...
    report: dict[str, object] = {"ok": True, "version": version, "schema": SCHEMA_VERSION}
    report["ok"] = bool(report["ok"]) and version == EXPECTED_VERSION and SCHEMA_VERSION == EXPECTED_SCHEMA

    integrity = check_release_integrity(root)
    report["release_integrity"] = integrity
    report["ok"] = bool(report["ok"]) and integrity.get("ok") is True

    try:
        policy = load_policy(root)
        report["policy_loaded"] = True
        report["policy_version"] = policy.get("version")
        report["adaptive_policy_present"] = isinstance(policy.get("adaptive_token_budget_policy"), dict)
        report["ok"] = bool(report["ok"]) and policy.get("version") == EXPECTED_VERSION and report["adaptive_policy_present"] is True
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        report["required_tables_present"] = REQUIRED_TABLES <= tables
        report["ok"] = bool(report["ok"]) and versions == list(range(1, EXPECTED_SCHEMA + 1)) and fk == 1 and report["required_tables_present"] is True

    commands = command_registry()
    tools = [str(t["name"]) for t in ALL_TOOLS]
    context_tools = {t for t in tools if t.startswith("agentos.context_")}
    ...
    report["context_mcp_tools"] = sorted(context_tools)
    report["context_mcp_exact"] = context_tools == EXPECTED_CONTEXT_MCP
    report["ok"] = bool(report["ok"]) and report["cli_unique"] is True and report["mcp_unique"] is True and report["context_mcp_exact"] is True

    if not args.skip_manifest:
        manifest = verify_manifest(root)
        report["manifest"] = manifest
        report["ok"] = bool(report["ok"]) and manifest["ok"] is True and manifest.get("release") == EXPECTED_VERSION
