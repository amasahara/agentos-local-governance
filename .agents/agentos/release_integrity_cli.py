...
Purpose:
    Expose current release-integrity and aggregate documentation gates.
...
def main() -> None:
    """Run a current release gate and exit fail-closed on findings."""
    p=argparse.ArgumentParser()
    p.add_argument("--root", type=Path, required=True)
    p.add_argument("--mode", choices=("integrity","docs"), default="integrity")
    args=p.parse_args()
    result = docs_check_current(args.root) if args.mode == "docs" else check_release_integrity(args.root)
