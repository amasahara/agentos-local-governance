...
def _merge_tools() -> tuple[list[dict[str, Any]], set[str], set[str]]:
    ...
    for definition in [*CORE_TOOLS, *FEATURE_TOOLS, *V0252_TOOLS, *V0253_TOOLS, *V0254_TOOLS, *V0255_TOOLS, *V0260_TOOLS, *V0261_TOOLS, *V0262_TOOLS, HEALTH_TOOL]:
...
def _health(root: Path, task_id: str | None, session_id: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "extension_readonly_tool_count": len(FEATURE_TOOLS) + len(V0252_TOOLS) + len(V0253_TOOLS) + len(V0254_TOOLS) + len(V0255_TOOLS) + len(V0260_TOOLS) + len(V0261_TOOLS) + len(V0262_TOOLS) - 1,
